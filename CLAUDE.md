...
5. **После каждого этапа ОБЯЗАТЕЛЬНО:**
   1. Полностью собрать проект и провалидировать: `npm run validate`, исправить ошибки
...
Проект использует **Turborepo** для управления monorepo — автоматический порядок задач, кэширование, параллелизация.

**Преимущества:**
- ✅ Автоматический topological order
- ✅ Кэширование (повторный запуск ~1-2 сек)
- ✅ Параллелизация независимых задач
- ✅ Запуск только измененных пакетов

**Конфигурация:** `turbo.json`, кэш в `.turbo/` (gitignore)

**Фильтры:**
```bash
# Только yandex-tracker и его зависимости
turbo run build --filter=@mcp-server/yandex-tracker

# Только измененные пакеты
...
turbo run build --verbose  # подробный вывод
