/**
 * Unit тесты для GeminiConnector
 *
 * Тестовые сценарии:
...
describe('GeminiConnector', () => {
  ...
  describe('getClientInfo()', () => {
    ...
    it('должен вернуть правильный путь к конфигу', () => {
      ...
      // Assert
      expect(info.configPath).toContain('.gemini/settings.json');
      expect(info.configPath).toBe(path.join(os.homedir(), '.gemini/settings.json'));
    });
  });
