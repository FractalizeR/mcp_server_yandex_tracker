...
import { QwenConnector } from '#cli/connectors/qwen/qwen.connector.js';
...
describe('QwenConnector', () => {
  ...
  describe('getClientInfo()', () => {
    it('должен вернуть корректную информацию о клиенте', () => {
      ...
      expect(info.description).toContain('Qwen');
      expect(info.platforms).toEqual(['darwin', 'linux', 'win32']);
      expect(info.configPath).toBeDefined();
    });

    it('должен вернуть правильный путь к конфигу', () => {
      // Act
      const info = connector.getClientInfo();

      // Assert
      expect(info.configPath).toContain('.qwen/settings.json');
      expect(info.configPath).toBe(path.join(os.homedir(), '.qwen/settings.json'));
    });
  });
