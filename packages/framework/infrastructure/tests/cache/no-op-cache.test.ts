...
describe('NoOpCache', () => {
  ...
  describe('set', () => {
    ...
    it('не должен бросать ошибку при установке с TTL', async () => {
      await expect(cache.set('key', 'value', 5000)).resolves.not.toThrow();
    });
    ...
    it('должен работать с любыми типами данных', async () => {
      await expect(cache.set('string', 'value')).resolves.not.toThrow();
      await expect(cache.set('number', 123)).resolves.not.toThrow();
      await expect(cache.set('object', { foo: 'bar' })).resolves.not.toThrow();
      await expect(cache.set('array', [1, 2, 3])).resolves.not.toThrow();
      await expect(cache.set('null', null)).resolves.not.toThrow();
    });
  });
