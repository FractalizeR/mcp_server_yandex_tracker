...
- [x] package.json framework/cli корректен
- [x] dependency-cruiser правила обновлены
- [x] `npm run depcruise` проходит (2 warnings внутри CLI пакета - не критично)
- [x] `npm run build` проходит во всех пакетах
- [x] `npm test:quiet` проходит во всех пакетах
- [x] Граф зависимостей валиден

---

## Критерии готовности

- [x] CLI зависит только от infrastructure
- [x] Нет критических циклических зависимостей (только внутри CLI)
- [x] Все пакеты собираются в правильном порядке (Turborepo)
- [x] Тесты проходят во всех пакетах
